...
export function isPublicPath(pathname: string): boolean {
  return PUBLIC_PATHS.some((base) => isUnder(pathname, base));
}
 ...
 *
 * `/` matches **only** itself. A prefix test would light up the Home tab on
 * every route in the app, since every path starts with a slash. Everything
 * else matches its own path and anything nested under it, so the workout
